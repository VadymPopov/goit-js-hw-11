...
function onFormSubmit(evt) {
    ...
    if (userInput === "") {
        (0, _notiflixNotifyAio.Notify).info("Search line can't be empty, try again");
        return;
    }
    (0, _fetchImages.getImages)(userInput, page).then((data)=>searchRequest(data)).catch((err)=>onFetchError(err));
}
function searchRequest(arr) {
    if (page === 1 && arr.hits.length > 1) (0, _notiflixNotifyAio.Notify).success(`Hooray! We found ${arr.totalHits} images.`);
...
function onLoadBtnClick() {
    ...
    (0, _fetchImages.getImages)(input, page).then((data)=>LoadMoreRequest(data)).catch((err)=>onFetchError(err));
}
...
},{"notiflix/build/notiflix-notify-aio":"eXQLZ","simplelightbox/dist/simple-lightbox.min.css":"kaxSc","simplelightbox":"9ydBq","./js/fetchImages":"dOg5x","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"eXQLZ":[function(require,module,exports) {
var global = arguments[3];
/*
...
async function getImages(userInput, page) {
    ...
    const response = await (0, _axiosDefault.default).get(`${BASE_URL}?key=${key}&q=${userInput}&${searchParams}`);
